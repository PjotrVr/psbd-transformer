...
def _disk_kernel(radius: int, alias_blur: float) -> torch.Tensor:
    ...
    antialiased = _separable_blur(aliased.view(1, 1, *aliased.shape), weights, "reflect")
...
def _rgb_to_hsv(images: torch.Tensor) -> torch.Tensor:
    ...
    hue = torch.where(
        highest == red,
        blue_gap - green_gap,
        torch.where(highest == green, 2.0 + red_gap - blue_gap, 4.0 + green_gap - red_gap),
    )
    hue = (hue / 6.0) % 1.0
...
def _directional_blur(
    ...
    point = (width * math.sin(math.radians(angle)), width * math.cos(math.radians(angle)))
...
def _plasma_fractal(batch: int, map_size: int, wibble_decay: float) -> np.ndarray:
    ...
    while step >= 2:
        ...
        left = centres + np.roll(centres, 1, axis=1) + corners + np.roll(corners, -1, axis=2)
        maparray[:, 0:map_size:step, step // 2 : map_size : step] = wibbled_mean(left)
        top = centres + np.roll(centres, 1, axis=2) + corners + np.roll(corners, -1, axis=1)
...
def impulse_noise(images: torch.Tensor, severity: int) -> torch.Tensor:
    ...
    corrupted = torch.where(
        flipped, torch.where(salted, torch.ones_like(images), torch.zeros_like(images)), images
    )
...
def snow(images: torch.Tensor, severity: int) -> torch.Tensor:
    ...
    flakes = torch.randn(batch, 1, height, width, device=images.device) * spread + location
    flakes = _clipped_zoom(flakes, zoom_factor)
    flakes = torch.where(flakes < cutoff, torch.zeros_like(flakes), flakes).clamp(0.0, 1.0)
...
def fog(images: torch.Tensor, severity: int) -> torch.Tensor:
    strength, decay = ((1.5, 2), (2.0, 2), (2.5, 1.7), (2.5, 1.5), (3.0, 1.4))[severity - 1]
