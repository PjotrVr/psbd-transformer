...
itself lives in psbd.scores, on the CPU side. This module only produces the 2
forward-pass ingredients it needs.

Nothing here ever touches the model's own dropout modules. The perturbation comes
entirely from modules plugged in by psbd.positions, which live in hook closures
...
from .positions import activate_model_dropout, restore_model_dropout
...
def compute_dropout_pass_probs(
    ...
    The perturbation comes from modules plugged in by hooks (see psbd.positions),
