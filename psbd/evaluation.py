...
form of the same rule psbd.decision.detection_report applies inside a sweep. They
...
interchangeable: this one uses torch.quantile and psbd.decision uses numpy's, and
...
from .eval_loaders import build_clean_loader, build_poisoned_loader
...
def attack_success_rate(
    ...
    The loader must be built over psbd.poisoning.AttackSuccessSet, which selects
...
def evaluate_attack(
    ...
    BadNetConfig(label_mode="all_to_all") from psbd.attacks.badnet, or whatever
    psbd.attacks.default_config(attack_name) returns. Applies the trigger to every
