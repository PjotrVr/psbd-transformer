...
A defence is only interesting if it survives an adversary who knows it. PSBD
flags samples whose prediction shift under dropout is LOW, so an attacker who
controls training can add a penalty that pushes the poisoned samples' shift up
onto the clean distribution, keeping the backdoor while removing the statistic
the defence reads.
    ...
    restated
        train as usual, plus a hinge that is zero once the poisoned samples shift
        at least as much as the clean ones on average. It pushes the poisoned
        distribution up onto the clean one and then stops.

The hinge is deliberate. A symmetric |mean_C - mean_P| would also punish poisoned
PSU for rising above clean, which costs capacity for no evasion benefit and
risks overshooting into a separation the defence could read from the other side.

PSU here is the fractional form (matching the defender's preferred score):

    PSU_ratio(x) = 1 - mean_over_passes(P_c(x; p, theta'_i)) / P_c(x; theta)
...
See docs/plans/adaptive-attacker-and-dropout-stacking.md for the threat model,
the success criteria, and the transfer test that is the actual point.
"""
...
from defences.perturbations import build_perturbation
...
def psu_for_batch(model, images, probe, passes: int, logits=None):
    """Per-sample fractional PSU under the probe, differentiable w.r.t. the model.

    Returns (psu, logits) so the caller can reuse the unperturbed forward for
    cross-entropy. That reuse is not a micro-optimisation: every retained forward
    is a full ViT-B/16 activation graph, and computing the clean pass twice cost
    an entire graph and was enough on its own to run a batch of 64 out of memory
    on a 40GB card.
    ...
    depth; on ViT (dropout=0) it is a no-op. The probe modules are outside the
    model tree and explicitly set to train mode by plug_dropout, so they sample
    masks regardless.

    The probe is plugged and unplugged around the stochastic passes. The finally
    is not decoration: a probe left attached leaks into the next step's clean
    forward pass and compounds across steps, so the model would be trained
    against a perturbation nobody recorded.
    """
    ...
    if logits is None:
        logits = model(images)
    tracked = logits.argmax(dim=1).detach()
    ...
    base_clamped = base.clamp_min(1e-6)
    psu = (base - dropped) / base_clamped
    return psu, logits
...
def calibrate_probe_rate(
    ...
    (psbd_metrics.select_rate_at_matched_shift), so the attacker optimizes at
    the same perturbation strength the defender would choose.

    Runs one forward pass per candidate rate on the validation set, so the cost
    is small relative to a 15-epoch training run.
    """
    model.eval()
    ...
    baseline_argmax = torch.cat(baseline_argmax)
    ...
    for rate in candidate_rates:
        ...
        try:
            ...
            perturbed_argmax = torch.cat(perturbed_argmax)
...
def evasion_penalty(psu: torch.Tensor, is_poisoned: torch.Tensor) -> torch.Tensor:
    ...
    Returns a zero with grad history when either group is missing from the batch,
    so a batch that happens to contain no poisoned sample contributes nothing
    rather than producing a nan that silently poisons the running loss. At a 1%
    poison rate and batch 128 that is a common case, not an edge case.
    """
    poisoned = is_poisoned.bool()
    if poisoned.all() or not poisoned.any():
        return psu.sum() * 0.0
    return torch.relu(psu[~poisoned].mean() - psu[poisoned].mean())


def evasive_update(
    model, images, labels, is_poisoned, criterion, optimizer, probe, weight, passes
    ...
    # than a hidden backdoor, and the gap alone cannot tell the two apart.
    return loss.detach(), {
...
def train_one_epoch_evasive(
    model, loader, criterion, optimizer, device, probe, weight, passes
) -> tuple[float, dict]:
    ...
    return total_loss / max(batches, 1), means
