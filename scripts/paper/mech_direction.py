...
ABLATIONS = (("direction", "rank-1 direction"), ("random_dir_0", "random direction"), ("top_20", "top-20 coordinates"), ("random_20", "random-20 coordinates"))
CRYSTALLISED = 0.5
...
def neuron_records(results_dir: str) -> dict[str, dict]:
    ...
    records = {}
    for path in sorted(glob.glob(os.path.join(results_dir, "vit_*", "backdoor_neurons.json"))):
...
def main() -> None:
    ...
    ablation = [row for row in load_json(ablation_path) or [] if is_panel_folder(row["folder_name"])]
    persistence = load_json(persistence_path) or []
    inputs = [f"{args.results_dir}/vit_cifar10_*/backdoor_neurons.json ({len(records)})", ablation_path, persistence_path]

    # Figure: 3 depth profiles per attack with the benign reference dashed.
    fig, axes = plt.subplots(1, 3, figsize=(11, 3.6))
    plotted = {}
    keys = (("rel_direction_norm", "relative backdoor-direction norm"), ("tac_mean", "mean trigger-activated change"), ("cka", "clean-versus-triggered CKA"))
    for index, (attack, record) in enumerate(sorted(records.items())):
        layers = [row["layer"] for row in record["layers"]]
        plotted[attack] = {key: [row[key] for row in record["layers"]] for key, _ in keys}
        for axis, (key, label) in zip(axes, keys):
            style = {"color": "0.5", "linestyle": "--"} if attack == "benign" else {"color": OKABE_ITO[index % len(OKABE_ITO)]}
            axis.plot(layers, plotted[attack][key], marker="o", markersize=3, linewidth=1.3, label=attack, **style)
    ...
    for row in sorted(ablation, key=lambda r: r["attack"]):
        cells = [row["attack"], str(row["peak_layer"]), fmt(row["baseline"]["asr"]), fmt(row["baseline"]["clean_accuracy"])]
        for key, _ in ABLATIONS:
            block = row["ablated"].get(key)
            cells.append(f"{fmt(block['asr'])} / {fmt(block['clean_accuracy'])}" if block else "--")
        rows.append(cells)
    write_table(
        ...
        header=["attack", "peak layer", "ASR", "CA", *[label for _, label in ABLATIONS]],
        rows=rows,
        align="lrrrrrrr",
    )
    ...
    for record in persistence:
        ...
        layers_aligned = [index + 1 for index, value in enumerate(alignment) if value is not None and value >= CRYSTALLISED]
        if not layers_aligned:
            continue
        crystal.setdefault(record["attack"], []).append(min(layers_aligned))
    for attack, layers in sorted(crystal.items()):
        crystal_rows.append([attack, str(len(layers)), fmt(mean_or_none(layers), places=1), str(min(layers)), str(max(layers))])
    write_table(
    ...
    macros = {
        "direction_checkpoints": (str(len(records)), "CIFAR-10 checkpoints with a backdoor_neurons record, benign included"),
        "direction_benign_peak_rel_norm": (fmt(max((row["rel_direction_norm"] for row in benign), default=None), places=2), "peak relative backdoor-direction norm on the benign reference under the same trigger"),
        "direction_backdoored_min_final_rel_norm": (
            fmt(min((record["layers"][-1]["rel_direction_norm"] for attack, record in records.items() if attack != "benign"), default=None), places=2),
            "smallest last-layer relative direction norm over the backdoored CIFAR-10 checkpoints",
        ),
        "ablation_checkpoints": (str(len(direction_rows)), "checkpoints in the causal ablation outside the SAM set"),
        "ablation_direction_max_asr": (fmt(max(row["ablated"]["direction"]["asr"] for row in direction_rows if row["attack"] != "badnet_a2a"), places=2), "highest attack success left after removing the rank-1 direction, all-to-one attacks"),
        "ablation_top_coordinates_min_asr": (fmt(min(row["ablated"]["top_20"]["asr"] for row in direction_rows if row["attack"] != "badnet_a2a"), places=2), "lowest attack success left after zeroing the top-20 coordinates, all-to-one attacks"),
        "ablation_random_direction_min_asr": (fmt(min(row["ablated"]["random_dir_0"]["asr"] for row in direction_rows if row["attack"] != "badnet_a2a"), places=2), "lowest attack success left after removing a random direction, all-to-one attacks"),
        "crystallisation_badnet_layer": (fmt(mean_or_none(crystal.get("badnet_a2o", [])), places=1), "mean crystallisation layer of badnet_a2o on CIFAR-100 and Tiny"),
        "crystallisation_blend_layer": (fmt(mean_or_none(crystal.get("blend", [])), places=1), "mean crystallisation layer of blend on CIFAR-100 and Tiny"),
        "crystallisation_checkpoints": (str(sum(map(len, crystal.values()))), "checkpoints in the crystallisation record"),
    }
    write_macros(os.path.join(args.paper_dir, "tables", "direction.macros.json"), GENERATOR, inputs, macros)
    print(f"direction: {len(records)} records, ablation {len(direction_rows)}, crystal { {k: len(v) for k, v in crystal.items()} }")
