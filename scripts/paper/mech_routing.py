...
def ordered_attacks(names: set[str]) -> list[str]:
    ...
    ordered = [name for name in ranked if name in names] + sorted(name for name in names if name not in ranked)
    return ordered


def records_by_attack(results_dir: str, name: str, folders: list[str]) -> dict[str, list[dict]]:
    grouped: dict[str, list[dict]] = {}
    for folder in folders:
        record = load_json(os.path.join(results_dir, folder, name))
        if record is None:
            continue
        attack = "benign" if "benign" in folder else record.get("attack", folder.split("_")[2])
        grouped.setdefault(attack, []).append(record)
    return grouped


def layer_mean(records: list[dict], key: str, layers: tuple[int, ...]) -> float | None:
    values = [row[key] for record in records for row in record["layers"] if row["layer"] in layers and row.get(key) is not None]
...
def main() -> None:
    ...
    clearing = [cell["folder_name"] for cell in clearing_cells(load_coverage(args.results_dir))]
    benign = list(declaration["benign_reference"].values())

    routing = records_by_attack(args.results_dir, "cls_routing.json", all_folders(args.results_dir, "cls_routing.json"))
    attribution = records_by_attack(args.results_dir, "logit_attribution.json", all_folders(args.results_dir, "logit_attribution.json"))
    artifacts = records_by_attack(args.results_dir, "artifact_tokens.json", clearing + benign)
    ...
    for index, attack in enumerate(attacks):
        ...
        left.plot(range(1, len(curve) + 1), curve, marker="o", markersize=3, linewidth=1.3, color=OKABE_ITO[index % len(OKABE_ITO)], label=f"{attack} (n={len(routing[attack])})")
    if "benign" in routing:
        curve = layer_curve(routing["benign"], "weight_backdoor")
        plotted["cls_attention_on_trigger"]["benign"] = curve
        left.plot(range(1, len(curve) + 1), curve, color="0.5", linestyle="--", linewidth=1.2, label="benign, same trigger")
    ...
    for index, attack in enumerate(artifact_attacks):
        ...
        right.plot(range(1, len(curve) + 1), curve, marker="o", markersize=3, linewidth=1.3, color=OKABE_ITO[index % len(OKABE_ITO)], label=f"{attack} (n={len(artifacts[attack])})")
    if "benign" in artifacts:
        curve = layer_curve(artifacts["benign"], "trigger_rank_triggered")
        plotted["trigger_norm_rank"]["benign"] = curve
        right.plot(range(1, len(curve) + 1), curve, color="0.5", linestyle="--", linewidth=1.2, label="benign, same trigger")
    ...
    rows = []
    stats = {}
    for attack in ordered_attacks((set(routing) | set(artifacts)) - {"benign"}) + ["benign"]:
        route = routing.get(attack, [])
        attr = attribution.get(attack, [])
        art = artifacts.get(attack, [])
        sink = sinks.get(attack, [])
        attention_write = sum(value for value in (layer_mean(attr, "attention_write_delta", (layer,)) for layer in range(1, 13)) if value is not None) if attr else None
        mlp_write = sum(value for value in (layer_mean(attr, "mlp_write_delta", (layer,)) for layer in range(1, 13)) if value is not None) if attr else None
        ...
        stats[attack] = {
            ...
            "attention_share": (attention_write / (attention_write + mlp_write)) if attention_write is not None and mlp_write is not None and (attention_write + mlp_write) else None,
            "rank_routing": layer_mean(art, "trigger_rank_triggered", ROUTING_LAYERS) if token_level else None,
            "rank_late": layer_mean(art, "trigger_rank_triggered", LATE_LAYERS) if token_level else None,
            "ratio_peak": max((row["ratio_triggered"] for record in art for row in record["layers"] if row["layer"] in NORM_LAYERS), default=None),
            "ratio_clean_peak": max((row["ratio_clean"] for record in art for row in record["layers"] if row["layer"] in NORM_LAYERS), default=None),
            "takeover_routing": layer_mean(sink, "takeover_fraction", ROUTING_LAYERS) if token_level else None,
            "token_share": share,
        }
        s = stats[attack]
        rows.append(
            [
                attack,
                "control" if attack == "benign" else ("local" if attack in LOCAL_ATTACKS else "global"),
                f"{len(route)}/{len(art)}",
                fmt(s["cls_early"]),
                fmt(s["cls_routing"]),
                fmt(s["cls_late"]),
    ...
    write_table(
        ...
        caption=(
            ...
            "ratio from layer 3 on (artifact\\_tokens.json, every clearing cell), and "
            "the takeover fraction over the routing band, how much attention mass "
            "other tokens absorb when the trigger's are masked (sink\\_anatomy.json). "
            "A global trigger covers every patch, so its token-level columns are "
            "absent. n is routing/artifact cells."
        ),
        label="tab:routing",
        header=["attack", "trigger", "n", "CLS mass 1-4", "CLS mass 5-8", "CLS mass 9-12", "attn share", "rank 5-8", "rank 9-12", "peak ratio", "takeover 5-8"],
        rows=rows,
        align="llrrrrrrrrr",
    )
    ...
    macros = {
        "routing_cells": (str(sum(map(len, routing.values()))), "checkpoints with a routing record, benign included"),
        "routing_badnet_cls_early": (fmt(badnet.get("cls_early")), "badnet_a2o CLS attention mass on the trigger tokens, layers 1 to 4"),
        "routing_badnet_cls_routing": (fmt(badnet.get("cls_routing")), "badnet_a2o CLS attention mass on the trigger tokens, layers 5 to 8"),
        "routing_badnet_cls_late": (fmt(badnet.get("cls_late")), "badnet_a2o CLS attention mass on the trigger tokens, layers 9 to 12"),
        "routing_benign_cls_late": (fmt(stats.get("benign", {}).get("cls_late")), "benign CLS attention mass on the same trigger tokens, layers 9 to 12"),
        "routing_badnet_attention_share": (fmt(badnet.get("attention_share"), places=2), "badnet_a2o attention share of the summed target-logit attribution"),
        "routing_global_attention_share": (fmt(mean_or_none([s["attention_share"] for s in global_ if s["attention_share"] is not None]), places=2), "mean attention share of the target-logit attribution over global-trigger attacks"),
        "routing_local_rank_routing": (fmt(mean_or_none([s["rank_routing"] for s in local if s["rank_routing"] is not None]), places=2), "mean trigger-token norm rank over the routing band, local-trigger attacks"),
        "routing_benign_rank_routing": (fmt(stats.get("benign", {}).get("rank_routing"), places=2), "benign trigger-token norm rank over the routing band under the same trigger"),
        "routing_benign_rank_late": (fmt(stats.get("benign", {}).get("rank_late"), places=2), "benign trigger-token norm rank over the late band under the same trigger"),
        "routing_badnet_takeover": (fmt(badnet.get("takeover_routing"), places=2), "badnet_a2o takeover fraction over the routing band"),
        "routing_benign_takeover": (fmt(stats.get("benign", {}).get("takeover_routing"), places=2), "benign takeover fraction over the routing band"),
        "routing_benign_ratio_peak": (fmt(stats.get("benign", {}).get("ratio_clean_peak"), places=2), "benign peak max-over-median patch-norm ratio on clean images, layer 3 on"),
        "routing_badnet_ratio_peak": (fmt(badnet.get("ratio_peak"), places=2), "badnet_a2o peak max-over-median patch-norm ratio on triggered images, layer 3 on"),
        "routing_global_cls_routing": (fmt(mean_or_none([s["cls_routing"] for s in global_ if s["cls_routing"] is not None])), "mean CLS attention mass on the trigger tokens over the routing band, global-trigger attacks"),
    }
    write_macros(os.path.join(args.paper_dir, "tables", "routing.macros.json"), GENERATOR, inputs, macros)
    print("routing: " + ", ".join(f"{a}: cls5-8 {fmt(s['cls_routing'])} rank5-8 {fmt(s['rank_routing'], 2)}" for a, s in stats.items()))
