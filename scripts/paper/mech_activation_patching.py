...
GROUP_LABELS = {
    ...
    "cls": "class token",
    "random_same_size": "random tokens, same count",
}
...
def mean_curves(records: list[dict]) -> dict[str, list[float]]:
    ...
    layers = sorted(
        {layer for record in records for layer in recovery_by_layer(record)["trigger"]}
    )
    averaged = {}
    for group in GROUPS:
        averaged[group] = [
            mean_or_none(
                [
                    recovery_by_layer(record)[group][layer]
                    for record in records
                    if layer in recovery_by_layer(record)[group]
                ]
            )
            for layer in layers
        ]
...
def last_full_layer(curves: dict[str, list[float]]) -> int | None:
    ...
    kept = [
        layer
        for layer, value in zip(curves["layers"], curves["trigger"])
        if value is not None and value >= RECOVERED
    ]
...
def main() -> None:
    ...
    for cell in cells:
        record = load_json(
            os.path.join(
                args.results_dir, cell["folder_name"], "activation_patching.json"
            )
        )
    ...
    inputs = [
        coverage_path,
        f"{args.results_dir}/<folder>/activation_patching.json ({sum(map(len, by_attack.values()))} cells)",
    ]
    ...
    for axis, attack in zip(axes.flat, attacks):
        ...
        axis.set_title(
            f"{attack_label(attack)} (n={len(by_attack[attack])})", fontsize=9
        )
    ...
    figure_sidecar(
        figure_path.replace(".pdf", ".json"),
        GENERATOR,
        inputs,
        {
            "curves": curves,
            "cells_per_attack": {a: len(r) for a, r in by_attack.items()},
        },
    )
    ...
    for attack in attacks:
        curve = curves[attack]
        random_max = max(
            value for value in curve["random_same_size"] if value is not None
        )
        early_trigger = mean_or_none(
            [
                value
                for layer, value in zip(curve["layers"], curve["trigger"])
                if layer <= EARLY_LAST_LAYER and value is not None
            ]
        )
        final_cls = curve["cls"][-1]
        stats[attack] = {
            "crossover": crossover_layer(curve),
            "last_full": last_full_layer(curve),
        ...
        rows.append(
            [
                ...
                fmt(early_trigger),
                str(stats[attack]["last_full"])
                if stats[attack]["last_full"] is not None
                else "--",
                str(stats[attack]["crossover"])
                if stats[attack]["crossover"] is not None
                else "--",
                fmt(final_cls),
                fmt(random_max),
            ]
    ...
    write_table(
        ...
        inputs=inputs,
        caption=(
            "Activation patching at the residual stream, averaged over the models "
            "whose attack succeeded, per attack: mean recovery when the trigger's own "
            "tokens are patched over "
        ...
        header=[
            ...
            "trigger",
            "n",
            "trigger tokens, layers 1 to 6",
            "last layer trigger tokens suffice",
            "crossover layer",
            "class token, last layer",
            "random null, max",
        ],
        rows=rows,
        align="llrrrrrr",
    ...
    macros = {
        ...
        "patching_early_layers": (
            ...
            "the layer band the early trigger-token recovery is averaged over",
        ),
        "patching_badnet_last_full": (
            str(stats.get("badnet_a2o", {}).get("last_full")),
            "badnet_a2o last layer at which the trigger tokens alone recover at least half the clean answer",
        ),
        "patching_random_null_max": (
            fmt(max(s["random_max"] for s in stats.values())),
            "largest recovery any random token group reaches, over every attack and layer",
        ),
        "patching_badnet_early_trigger": (
            fmt(stats.get("badnet_a2o", {}).get("early_trigger")),
            "badnet_a2o mean recovery from the trigger tokens over layers 1 to 6",
        ),
        "patching_badnet_crossover": (
            str(stats.get("badnet_a2o", {}).get("crossover")),
            "badnet_a2o layer at which the class token first recovers more than the trigger tokens",
        ),
        "patching_badnet_final_cls": (
            fmt(stats.get("badnet_a2o", {}).get("final_cls")),
            "badnet_a2o class-token recovery at the last layer",
        ),
        "patching_local_early_trigger": (
            fmt(mean_or_none([s["early_trigger"] for s in local])),
            "mean early trigger-token recovery over the local-trigger attacks",
        ),
        "patching_global_early_trigger": (
            fmt(mean_or_none([s["early_trigger"] for s in global_])),
            "mean early trigger-token recovery over the global-trigger attacks",
        ),
        "patching_local_crossover": (
            fmt(
                mean_or_none(
                    [s["crossover"] for s in local if s["crossover"] is not None]
                ),
                places=1,
            ),
            "mean crossover layer over the local-trigger attacks",
        ),
        "patching_blend_early_trigger": (
            fmt(stats.get("blend", {}).get("early_trigger")),
            "blend mean recovery from the trigger tokens over layers 1 to 6",
        ),
    }
    write_macros(
        os.path.join(args.paper_dir, "tables", "activation_patching.macros.json"),
        GENERATOR,
        inputs,
        macros,
    )
    print(
        "patching: "
        + ", ".join(
            f"{a}: cross {s['crossover']} last {s['last_full']} rand {fmt(s['random_max'])}"
