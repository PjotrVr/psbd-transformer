...
def main() -> None:
    ...
    for column, dataset in enumerate(DATASET_ORDER):
        rows = sorted(
            (r for r in records if r["dataset"] == dataset),
            key=lambda r: (
                order.index(r["attack"]) if r["attack"] in order else 99,
                r["folder"],
            ),
        )
    ...
    for item in sorted(
        records,
        key=lambda r: (
            DATASET_ORDER.index(r["dataset"]) if r["dataset"] in DATASET_ORDER else 9,
            r["folder"],
        ),
    ):
    ...
    write_table(
        ...
        inputs=inputs,
        caption=(
            f"Per-layer profile of the backdoor direction on {len(records)} ViT-B/16 models, "
            f"{record['samples']} paired images each."
        ),
        label="tab:tac-layers",
        header=[
            "Dataset",
    ...
    macros = {
        "tac_layers_checkpoints": (
            str(len(records)),
            "checkpoints in the per-layer TAC pass",
        ),
        "tac_layers_samples": (
            str(record["samples"]),
        ...
        "tac_layers_benign_peak_max": (
            ...
            "largest peak relative direction norm over the benign references",
        ),
        "tac_layers_badnet_onset": (
            fmt(
                mean_or_none(
                    [o for o in onsets.get("badnet_a2o", []) if o is not None]
                ),
                places=1,
            ),
            "mean onset layer of badnet_a2o in the per-layer TAC pass",
        ),
        "tac_layers_blend_onset": (
            fmt(
                mean_or_none([o for o in onsets.get("blend", []) if o is not None]),
                places=1,
            ),
            "mean onset layer of blend in the per-layer TAC pass",
        ),
        "tac_layers_earliest_onset_attack": (
            min(
                ((mean_or_none([o for o in v if o is not None]) or 99), k)
        ...
        "tac_layers_latest_onset_attack": (
            max(
                ((mean_or_none([o for o in v if o is not None]) or 0), k)
                for k, v in onsets.items()
            )[1]
            if onsets
            else "--",
    ...
    write_macros(
        os.path.join(args.paper_dir, "tables", "tac_layers.macros.json"),
        GENERATOR,
        inputs,
        macros,
    )
    print(
        "tac layers: "
        + ", ".join(f"{k}: onset {v}" for k, v in onsets.items())
        + f", benign peaks {benign_peaks}"
    )
