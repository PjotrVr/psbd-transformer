...
class LDeiT(L.LightningModule):
    def __init__(
        ...
        self.model = deit_tiny(num_classes=num_classes)
...
def train_deit(
    ...
    best_model = LDeiT.load_from_checkpoint(best_checkpoint_path)
...
def main():
    parser = argparse.ArgumentParser(description="Train DeiT-Tiny on benign data")
    ...
    result = train_deit(
        ...
        run_name_prefix=f"benign_deit_tiny_{args.dataset_name}",
        num_classes=num_classes,
        config=config,
    )
