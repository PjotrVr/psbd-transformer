...
score by \StaircaseOperatorsBeforeAttentionNormGaussianGain{}. What decides the operator
is not the site but the side of the LayerNorm the perturbation lands on. Holding that side
...
\begin{enumerate}
  ...
  \item 2 refutations. The claim that dropout before the residual add beats
    ...
    models. Nor is the winner the input-side family. Held at token masking the
    family gap is \FamilyGapMatched{} [\FamilyGapMatchedLow{},
    \FamilyGapMatchedHigh{}] at matched disturbance, and held at dropout it
    reverses to \FamilyGapDropoutMatched{} [\FamilyGapDropoutMatchedLow{},
    \FamilyGapDropoutMatchedHigh{}]. A split whose sign follows the operator is
    not a fact about sites (\cref{sec:results:family}).
