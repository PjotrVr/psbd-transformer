...
\caption{One ViT block with the sites we perturb, in forward order. Site A is the attention input, where PSBD-TM masks whole tokens. Site C is the residual stream after the add, where PSBD-RD applies dropout. The attention branch output before the add (B), the MLP input (D), the attention heads and the MLP hidden units are the other sites in the basis.}
\label{fig:vit-block}
\end{figure}
