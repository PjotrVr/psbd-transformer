...
\caption{One ViT block with the sites we perturb, in forward order. Site A is the attention input, where we mask whole tokens. Site C is the residual stream after the add, the site that corresponds to the PSBD paper's dropout on ResNets. The attention branch output before the add (B), the MLP input (D), the attention heads and the MLP hidden units are the other sites in the basis.}
\label{fig:vit-block}
\end{figure}
