...
class LSwin(L.LightningModule):
    def __init__(
        ...
        self.model = swin_tiny(num_classes=num_classes)
...
def train_swin(
    ...
    best_model = LSwin.load_from_checkpoint(best_checkpoint_path)
...
def main():
    parser = argparse.ArgumentParser(description="Train Swin-Tiny on benign data")
    ...
    result = train_swin(
        ...
        run_name_prefix=f"benign_swin_tiny_{args.dataset_name}",
        num_classes=num_classes,
        config=config,
    )
