...
needs was written by psbd_dropout_sweep.py into results/<folder>/psbd/.

Output goes to results/<folder>/psbd_metrics.json, deliberately not metrics.json,
which metrics.py owns for baseline attack-success and clean-accuracy numbers.
...
def load_baselines(
    ...
    return {split: load_baseline(baseline_path(psbd_dir, split)) for split in SPLITS}
...
def analyze_one_rate(
    ...
    return {
        ...
        "clean_shift_to_target_fraction": _shift_to_target_fraction(
...
def _shift_to_target_fraction(
    histogram: list[int] | None, target_label: int | None
) -> float | None:
    if histogram is None or target_label is None:
        return None
    total = sum(histogram)
    return float(histogram[target_label] / total) if total > 0 else None
...
def analyze_position_config(
    ...
    """All rates for one placement, plus the two rate-selection verdicts."""
    rates = discover_rates(psbd_dir, position_config)
    by_rate = {
