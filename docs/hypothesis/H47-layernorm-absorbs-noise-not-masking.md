...
Some points are immediately followed by a LayerNorm and some are already normalized. LayerNorm
...
Survival ratio: relative change after the following normalization over relative change before
it. 1.000 means the perturbation passed through untouched. 2 checkpoints, 2 datasets,
...
tensor is already normalized the distinction vanishes entirely.
...
2. Mask minus gaussian is **+0.177 AUROC** (CI [+0.121, +0.239], n=67) at `before_attention_norm`,
   ...
   is already normalized. The gap does not shrink, it **reverses sign**.
...
reversal was gaussian failing at 1 position for the reason measured here. A single operator
