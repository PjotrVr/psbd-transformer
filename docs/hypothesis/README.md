...
| [H47](H47-layernorm-absorbs-noise-not-masking.md) | LayerNorm absorbs additive noise and not masking, so the operator ranking depends on position | **SUPPORTED**, closes H23's open secondary: `ln_1` passes 0.998 of a token mask and 0.808 of gaussian; mask-minus-gaussian is +0.177 where a norm follows and **-0.065 where it does not**, a sign reversal. Corrects this project's own 0.58x position/operator reading back to **1.53x** |
| [H48](H48-trigger-manufactures-its-own-sink.md) | The trigger manufactures an attention sink rather than hijacking one | **SUPPORTED** on three measurements: register neurons **disjoint** (0 of top 10, 0 of top 50), sink role **not replaceable** (takeover 0.13 to 0.36 at layers 5 to 12), and 64 spare test-time registers change **nothing** (ASR 1.000). This ViT has no native sink population at all (0.000% of tokens above 2x median), which also clears the artifact-token confound |
| [H49](H49-backdoor-is-routed-not-computed.md) | The backdoor lives in its own patches until layer 7 and in CLS after 9, and attention is what moves it | **SUPPORTED causally**: patching the 4 trigger tokens recovers the clean answer at **1.000** through layer 6, decaying to 0.188 at 12, while CLS rises 0.003 to **0.621**; random tokens of the same count recover **0.000** at every layer. The MLP writes 3.36x more aligned but is token-wise and can move nothing |
