...
| [H19](H19-placement-ranking-is-rate-selection.md) | The placement ranking is mostly about which placement the adaptive rate rule can aim at | **Swin half REFUTED** by its own follow-up data (0.11 gap on 6 units became 0.015 at 0.3 SE on 11); holds weakly on ViT. `blocks_9_12` still has the project's best oracle AUROC and **0/20 deployable operating points** |
| [H20](H20-input-side-beats-residual-adjacent.md) | What matters is sub-layer input versus residual stream, not pre- versus post-residual | **SUPPORTED**: input-side beats residual-adjacent by **+0.054** (4.0 SE, 11/12 units); pre-versus-post, the founding question, is **+0.002**. A single site at the embedding matches 12-site schemes on 3 attacks but has a 0.56 ceiling on the patch trigger |
