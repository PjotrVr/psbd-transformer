...
The paper's own adaptive section is the second failure mode, and it is measured rather than predicted. Eq. 5's scale-resistant regularization term pulls the amplified predictions of a poisoned image away from consistency during training, and the project's own cross-defense review reports this pushes SCALE-UP's AUROC to 0.467, below chance, with the clean-accuracy and ASR cost not reported by the paper's authors (`docs/attack-design/cross-defense.md`, Section 5). Since the attacker pays an unreported price, this is a documented weakness rather than a free evasion in the way STRIP's Section VI-F attack is, but the direction of the effect is the paper's own.
