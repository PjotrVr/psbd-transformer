...
`gain_scale` multiplies each channel by a random $\omega$ drawn from a scaled distribution, then adjusts LayerNorm bias. This amplifies any direction that deviates from the learned normalization statistics. On CIFAR-100, the backdoor direction stands out more strongly in the LayerNorm statistics because the 100-class decision boundary is more fragile. The amplification effect compounds across 12 transformer blocks. However, `gain_scale`'s TPR@5%FPR on Tiny BadNet at 1% is only 0.034 (near-random at the practical threshold), meaning the amplification also amplifies clean variation on a 200-class dataset. The ROC curve is steep at high FPR but flat at low FPR.
