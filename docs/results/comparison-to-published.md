...
after the residual add. The like-for-like comparison against competitor
detectors now lives in `docs/detectors/comparison.md`, generated by
...
|---|---|---|
| PSBD (Li et al., arXiv:2406.05826) | dropout @ pre_residual | This project's founding placement, not PSBD's own. The paper places dropout after the residual add (post_residual) on ResNet-18 |
...
| Attack | PSBD (ResNet-18, published) | dropout @ pre_residual (our founding placement) | token_mask @ before_attention_norm (our best) |
|---|---:|---:|---:|
...
Our ViT run at dropout @ pre_residual, this project's founding placement rather than PSBD's own, slightly underperforms on BadNet (0.919 vs 0.974) but matches on Blend (0.992 vs 0.998). Switching to our recommended position (token_mask @ before_attention_norm) recovers the BadNet gap and exceeds PSBD's ResNet on CIFAR-10.
...
The key finding: the published methods use fixed positions (post-residual for PSBD, BatchNorm for IBD-PSC, input pixels for SCALE-UP) that were chosen for ConvNets. Searching over 27 position/operator combinations on ViT reveals that:
...
4. **No single published method is optimal for ViT.** Our founding placement (dropout @ pre_residual, not PSBD's own post_residual) underperforms. IBD-PSC's operator is excellent but at a different position. SCALE-UP's pixel-space approach ranks only 10th. The grid search unifies them and finds the ViT-specific optimum.
