...
them already carrying A8's backfilled `poison_rate_capped` flag). On Tiny all 4 nominal rates resolve to the same 500 images, so its
clean-label "rate curve" is 4 replicates of 1 point. On CIFAR-100 and GTSRB, 2
of the 3 panel rates collapse onto 1.
...
picking a target picks a class they can actually poison. Clean-label runs on GTSRB
...
The docstring said so from the beginning. Turner et al. (2019) do not just stamp a
...
model trained on clean data, maximizing cross-entropy on each image's **true** label:
...
`ToTensor`, the trigger goes on and normalization comes last
([poison.py:236-247](../poison.py#L236-L247)). PGD therefore backpropagates through
normalization and through the model wrapper's own Resize to 224.

**1 surrogate serves every victim.** The bases are generated once per
...
construction. `AttackSuccessSet` already selects the eval trigger when it exists
...
the patch-only behavior byte for byte. Every LC checkpoint trained before this change
rebuilds through `default_config("lc")`, so a drift here would silently re-interpret
them. `tests/test_rewrite_library.py` pins the 2 attack trees bit-for-bit and
...
and refuses before training starts. 1 consequence: `--max-samples` is incompatible
...
not collapse, the bases carry no attack and no amount of training will produce an attack.
...
Folders follow the canonical template plus 2 new tags:
...
so only the faithful variant is a panel cell. 2 related fixes went in alongside:

- `scripts/coverage_ledger.py`'s `pivot_configs` keyed cells by `(dataset, rate) -> {attack: cell}`,
  so 2 cells sharing a slot silently overwrote each other by sort
  order. It now prefers the cell that clears the ASR bar and raises on a genuine tie
  rather than choosing arbitrarily between them.
...
disk. They are just read at their realized rate now, with replicate count, instead of
...
The 3 GTSRB LC cells read 0.464 / 0.239 / 0.129. Those spreads, 0.33 and 0.34, are
free replicates and they are the honest error bar on a single training run. A hard
0.85 ASR bar applied to 1 run is shaky at that spread, which is why the replacement
...
The pilot writes 1 folder per epsilon (`..._adv16_pilot`), and `_pilot` is an excluded
...
### 1 trap worth knowing, if you write the commands by hand
