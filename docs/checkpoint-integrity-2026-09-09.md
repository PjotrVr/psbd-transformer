...
failing test, and not found by anything else in the 4 weeks it had been happening:
...
Every generator is supposed to pass `checkpoints/<name>/attack_result.pt`. 2 passed
...
its own results as it produced them**, with 14 more jobs still queued, each several
...
**2. Fixed the 2 generators** (`pbs/generate_trigger_sweep_jobs.py:59`,
...
into `checkpoints/<name>/attack_result.pt` and rebuilds `args.json` from 2 sources
