...
The hardest setting on the hardest dataset. 3 configurations:
...
rather than 1 being quietly chosen:
...
The table below is the nearest-rate estimator. The interpolated estimator gives dropout
0.652 and token_mask 0.818, a gain of **+0.166**. Quoting either is defensible.
Quoting 1 number from 1 estimator and comparing it against the other is not, which is a
...
- **token_mask gains +0.216 by the nearest-rate estimator and +0.166 by the
  interpolated estimator**, and **+0.089 over the full 48-cell panel** (interpolated),
...
FPR actually achieved in brackets. `oracle` reads the same target off the labeled
...
4 changes assembled from earlier hypotheses:
...
PSBD, IBD-PSC, SCALE-UP and STRIP are 1 method: perturbation-consistency
...
### 4.1 1 linear direction, not a set of neurons (H16)
...
### 4.3 3 shared backdoor heads (H31)

3 attention heads diverge across ALL attacks and datasets: **L5H0, L6H3,
...
9 of 10 attacks are nearly perpendicular to the readout weight (87 to 91
...
2 observations from the earlier SAM work are retained because they are about the
...
`alpha * x + branch`. 3 distinct regimes:
...
4 versions of 1 pattern, each inverted a conclusion:
...
1. **Comparing placements at shared dropout rate (H9, H1, H3).** Different
   ...
   1 position is past the collapse point for another. Fixed by matching on
