...
> for the record. This placement is not the current recommended placement (`before_attention_norm_token_mask`), so its per-cell numbers below should not be read as the headline result.
