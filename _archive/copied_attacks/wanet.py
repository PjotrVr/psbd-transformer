...
class poison_generator:
    ...
    def generate_poisoned_training_set(self):
        ...
        print(
            f"backdoor samples num: {len(backdoor_indices)}, cover samples num: {len(cover_indices)}."
        )
        ...
        for i in backdoor_indices:
            ...
            img = F.grid_sample(img.unsqueeze(0), self.grid_temps, align_corners=True)[
                0
            ]
...
class poison_transform:  # Make poison samples
    def __init__(
        ...
        self.grid_temps = (
            identity_grid + s * noise_grid / self.img_size
        ) * self.grid_rescale
        self.grid_temps = torch.clamp(self.grid_temps, -1, 1)
    ...
    def transform(self, data, labels):
        ...
        data = F.grid_sample(
            data,
            self.grid_temps.to(data.device).repeat(data.shape[0], 1, 1, 1),
            align_corners=True,
        )
