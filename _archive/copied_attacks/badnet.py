...
class poison_generator:
    def __init__(
        self, img_size, dataset, poisoning_ratio, trigger, mask, target_label, alpha=1.0
    ):
        """
    ...
    def generate_poisoned_training_set(self):
        ...
        backdoor_maker = poison_transform(
            self.img_size, self.trigger, self.mask, self.target_label, self.alpha
        )
...
class poison_transform:  # Make poison samples
    ...
    def transform(self, imgs, labels):
        ...
        imgs = imgs + self.alpha * self.mask.to(imgs.device) * (
            self.trigger.to(imgs.device) - imgs
        )
