...
def prepare_dataset(args):
    ...
    if args.dataset == "cifar10":
        test_set = datasets.CIFAR10(
            root=test_dir, transform=data_transform, train=False
        )
    elif args.dataset == "gtsrb":
        test_set = GTSRB(
            root=test_dir,
        ...
        test_set = datasets.ImageFolder(root=test_dir, transform=data_transform)
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epochs + 1):  # Train backdoored base model
        ...
        print(
            "Benign Training> Train Epoch: {} \tLoss: {:.6f}, lr: {:.6f}, Time: {:.2f}s".format(
    ...
    if args.log:
        ...
        log_path = os.path.join(
            log_path,
            "%s" % (exe_start_time.strftime("%Y-%m-%d-%H:%M:%S") + "benign model"),
        )
