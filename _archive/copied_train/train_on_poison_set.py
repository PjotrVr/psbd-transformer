...
def add_arguments(parser):
    ...
    parser.add_argument(
        "-random_seed", type=int, required=False, default=default_args.random_seed
    )
    parser.add_argument(
        "-checkpoint_save_path",
        type=str,
        required=False,
        default=default_args.checkpoint_save_path,
    )
    parser.add_argument(
        "-checkpoint_save_epoch",
        type=int,
        required=False,
        default=default_args.checkpoint_save_epoch,
    )
    return parser
...
def prepare_dataset(args):
    ...
    if args.dataset == "cifar10":
        clean_test_set = datasets.CIFAR10(
            root=clean_test_dir, transform=data_transform, train=False
        )
        ...
        clean_test_set = datasets.ImageFolder(
            root=clean_test_dir, transform=data_transform
        )
    else:
    ...
    if not args.load_bench_data:
        ...
        if args.poison_type in ["lc"] and args.dataset == "tiny":
            poison_test_dir = os.path.join(
                os.path.dirname(poison_set_dir), "lc_backdoor_test"
            )
            backdoor_test_imgs_path = os.path.join(
                poison_test_dir, "backdoor_test_imgs.pt"
            )
            backdoor_test_labels_path = os.path.join(
                poison_test_dir, "backdoor_test_labels.pt"
            )
        ...
        clean_train_set = torch.load(
            os.path.join(poison_set_dir, "clean_train_data.pt")
        )
        backdoor_train_set = torch.load(
            os.path.join(poison_set_dir, "backdoor_train_data.pt")
        )
        backdoor_test_set = torch.load(
            os.path.join(poison_set_dir, "backdoor_test_data.pt")
        )
        ...
        for idx in range(len(backdoor_train_set)):
            ...
            transformed_backdoor_train_labels.append(
                attack_config.target_label[args.dataset]
            )

        transformed_backdoor_train_imgs = torch.stack(
            transformed_backdoor_train_imgs, dim=0
        )
        transformed_backdoor_train_labels = torch.tensor(
            transformed_backdoor_train_labels
        )
        ...
        clean_train_set = torch.utils.data.TensorDataset(
            transformed_clean_train_imgs, transformed_clean_train_labels
        )
        backdoor_train_set = torch.utils.data.TensorDataset(
            transformed_backdoor_train_imgs, transformed_backdoor_train_labels
        )
        backdoor_test_set = torch.utils.data.TensorDataset(
            backdoor_test_imgs, backdoor_test_labels
        )

    if args.poison_type not in ["wanet", "adaptive_blend"]:
        poisoned_train_set = torch.utils.data.ConcatDataset(
            [clean_train_set, backdoor_train_set]
        )
    else:
        poisoned_train_set = torch.utils.data.ConcatDataset(
            [clean_train_set, cover_train_set, backdoor_train_set]
        )
...
def get_checkpoint_path(args):
    ...
    checkpoint_path = os.path.join(
        checkpoint_path,
        str(args.poisoning_ratio),
        "random_seed_" + str(args.random_seed),
        args.poison_type,
    )
    ...
    checkpoint_path = os.path.join(
        checkpoint_path,
        "target_label_"
        + str(attack_config.target_label[args.dataset])
        + "_seed_"
        + str(attack_config.poison_seed),
    )
    ...
    if not os.path.exists(checkpoint_path):
        tools.create_missing_folders(checkpoint_path)
...
if __name__ == "__main__":
    ...
    poisoned_train_set, clean_train_set, clean_test_set, backdoor_test_set = (
    ...
    if args.resume > 0:
        ...
        if os.path.exists(checkpoint_save_path):
            model.load_state_dict(
                torch.load(
                    os.path.join(
                        checkpoint_save_path, "epoch_" + str(args.resume) + ".pt"
                    )
                )
            )
    ...
    for epoch in range(1, epochs + 1):  # Train backdoored base model
        ...
        if epoch >= args.checkpoint_save_epoch:
            torch.save(
                model.state_dict(),
                checkpoint_save_path + "/epoch_" + str(epoch) + ".pt",
            )
    ...
    if args.log:
        log_path = os.path.join(
            "../logs", "%s_random_seed=%d" % (args.dataset, args.random_seed)
        )
        if not os.path.exists(log_path):
            tools.create_missing_folders(log_path)
        log_path = os.path.join(
            log_path,
            "%s"
            % (
                exe_start_time.strftime("%Y-%m-%d-%H:%M:%S_")
                + supervisor.get_info_core(
                    args, include_poison_seed=attack_config.record_poison_seed
                )
            ),
        )
