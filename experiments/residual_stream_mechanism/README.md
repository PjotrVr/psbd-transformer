...
positions that are already normalized the distinction vanishes, exactly as predicted.
...
2. **Operator gap, position-dependent.** Mask minus gaussian is **+0.177 AUROC**
   ...
   normalized. The gap does not merely shrink, it **reverses sign**.
...
4. **Consequence for the position-versus-operator question.** Holding position fixed at
   ...
   published 1.43x. **The published claim survives. The apparent reversal was 1 pathological
...
survive normalization equally, so absorption is silent on the token-versus-channel question,
...
That is the downstream consequence, not the cause. The causal direction is the claim here.
