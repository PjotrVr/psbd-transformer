...
5 packages, each with its own `__init__.py`, plus a flat root for entrypoints and the shared attack/eval pipeline core that no single package owns:
...
- Repo root: `train_backdoor.py`, `train_benign.py`, `train.py`, `sam.py`, `metrics.py` (entrypoints and training/eval-pipeline code whose only real callers are the 2 training scripts, not reusable utilities or a distinct domain package) and `poison.py`, `backdoor_data.py`, `models.py` (shared by both attacks and defences-side eval code, or loaded from nearly everywhere the same way the entrypoints are, so not owned by any single package).
...
- Numbers as digits, never spelled out. Library packages and `cli/` are production code under `coding-style.md` (type hints, docstrings stating the contract, shape annotations on every tensor operation, a `return` never carries logic). `experiments/`, `notebooks/` and `scratch/` are research code under the same guide's research rules.
...
If all checks satisfy, run `ruff format` on the files you changed, never a repo-wide import sort.