...
The result that carries the paper is a placement. `token_mask` at `before_attention_norm` scores mean AUROC **0.927** at the adaptive 0.8 rule against **0.818** for the published ConvNet placement (dropout after the residual add), a paired gain of **+0.104**, **+0.110** on the hard attacks, with a floor of **0.418** and **2** inverted cells. The gain grows as the poison rate falls, which is the regime a defender needs.

Position and operator both matter. Gaussian noise minus token_mask at the attention input is **-0.183**, and the attention input minus the MLP input at the same operator is **+0.109**. The operator effect is decided by which side of a LayerNorm the perturbation lands on rather than by the site: held on the same side the interaction between the 2 axes cannot be separated from 0 ([Q19](../docs/open-questions.md)). The founding claim that dropout before the residual add beats dropout after it is refuted ([H1](../docs/hypothesis/H1-pre-beats-post.md), +0.002 at matched shift). So is the input-side against residual-adjacent family split, once the operator is held fixed.
...
**Read `docs/open-questions.md` before trusting any claim here.** It lists 33 claims the evidence does not support at the strength the repository asserts them, including that the declared selection protocol selects a different placement from the recommended placement. Full numbers in `paper/`, verdicts in `docs/hypothesis/README.md`.
