...
Research project adapting Prediction Shift Backdoor Detection (PSBD, Li et al., arXiv 2406.05826) from ConvNets to Vision Transformers (ViT-B/16 and Swin-S). Every headline number below is a macro in `paper/headline.tex`, generated by `scripts/paper/` from `results/coverage/coverage.json`. The values here are the ones that build produced on 2026-09-23. Do not hand-edit a number in this paragraph. Regenerate and read it off `paper/headline.tex`.

The panel holds **105** cells over cifar10, cifar100, gtsrb, tiny, svhn and eurosat. **71** clear the ASR bar 0.85 and **3** diverged during training. **69** of the clearing cells carry enough of the basis to be compared, over **6** datasets. `configs/psbd_basis.json` declares **27** placements and all **27** now reach the adaptive target somewhere on the panel, so a claim about "the basis" means the placements common to every compared cell and should say so. Dropout on the attention branch output was declared as `before_attention_residual_dropout`, a cache name `cache_config_name` never builds, so its 69 sweeps sat unfound under `before_attention_residual` and it read as declared-but-unswept for the whole project. `tests/test_canon.py` now holds every basis id to the name its own sweep would write.

The result that carries the paper is a placement. `token_mask` at `before_attention_norm` scores mean AUROC **0.927** at the adaptive 0.8 rule against **0.823** for the published ConvNet placement (dropout after the residual add), both over the same 69 cells, a paired gain of **+0.104**, **+0.110** on the hard attacks, with a floor of **0.418** and **2** inverted cells. The gain grows as the poison rate falls, **+0.073** at 10% to **+0.161** at 1%, which is the regime a defender needs. It transfers to Swin-S: **0.951** over **90** cells. Against the 11 ported competitor detectors on the same 69 models it ranks **1** of 13 on AUROC and the published placement ranks **5**, but the lead over the calibrated IBD-PSC port is **+0.013** AUROC and it trails that port by 0.002 at a 20% FPR budget, so it is a tie with the best competitor, not a new state of the art. On ViT the recommended placement is tied with its twin at the attention branch output, and the declared protocol's selection half picks the twin, so the recommendation rests on Swin, seed stability and the mechanism ([Q20](../docs/open-questions.md)).

Position and operator both matter. Gaussian noise minus token_mask at the attention input is **-0.128** at the adaptive rule (**-0.183** at the matched 0.6 rule), and the attention input minus the MLP input at the same operator is **+0.102**. The operator effect is decided by which side of a LayerNorm the perturbation lands on rather than by the site: held on the same side the interaction between the 2 axes cannot be separated from 0 ([Q19](../docs/open-questions.md)). The founding claim that dropout before the residual add beats dropout after it is refuted ([H1](../docs/hypothesis/H1-pre-beats-post.md), **+0.010** at matched shift, interval containing 0). The input-side against residual-adjacent family split is not a property of sites: held at token masking it is **+0.016** at matched shift and held at dropout it reverses to **-0.066**.

The PSBD paper's shift-to-target phenomenon exists on ViT for local triggers on few-class datasets and is absent for global triggers, while detection works in both regimes, so the neuron-bias account of the original paper is attack-dependent on ViT rather than the mechanism. The earlier +0.258 headline for `gain_scale` at `mlp_norm_out` stays withdrawn ([the audit](../docs/audit-2026-09-07.md)).

**Read `docs/open-questions.md` before trusting any claim here.** It lists the claims the evidence does not support at the strength the repository asserts them, with a status table at the top saying which are closed, which are only disclosed and which are open. Full numbers in `paper/`, verdicts in `docs/hypothesis/README.md`.
...
- `attacks/`: the 10 attacks (`badnet`, `blend`, `sig`, `wanet`, `lf`, `lc`, `bpp`, `adaptive_blend`, `tact` and `generated`), shared trigger `patterns`, `poisoning` (training and eval eligibility rules, index selection, `AttackSuccessSet`), `adversarial`, `bases` (Label-Consistent's adversarial bases) and `evasion`.
...
- `defenses/`: PSBD itself. `operators` (what is injected), `inference` (forward passes, `forward_logits`, `frozen_parameters`), `scores` (PSU and the shift ratio), `decision` (rate rules, quantile thresholds, `detection_report`, the canonical placements) and `cache` (the stage-1 tensors under `results/<folder>/psbd/`).
...
- PSBD threshold: 2000-sample clean validation set (`data.splits.PSBD_HELDOUT_SIZE`, seed `PSBD_SPLIT_SEED`), threshold at a quantile of clean validation scores (`defenses.decision.PSBD_QUANTILES`, headline 0.25). FPR is mechanically set by the quantile choice. Clean and backdoor eval sets are paired from the same images (`pair_clean_to_backdoor`). The 2-stage pipeline is `cli.sweep` (GPU, `--position` and `--operator`, writes raw per-pass probabilities under `results/<folder>/psbd/<placement>/` with a `run_<placement>.json` provenance record) and `cli.analyze` (CPU, reads the cache, writes `psbd_metrics.json`). The vocabulary is position (where), operator (what) and placement (the cache directory naming both), and `defenses.cache.read_run_provenance` translates the older `position_config` and `perturbation` keys of the caches on disk. Detection metrics live in `defenses.scores` and `defenses.decision`.
