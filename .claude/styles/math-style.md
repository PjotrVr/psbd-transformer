...
- Original (source) notation is the default. Descriptive form is produced only on request. This is the reverse of the code default, where descriptive is default
- A symbol table always accompanies a formula, regardless of how many symbols are involved
- Exception: symbol tables are for personal use, not for publication. When writing an official or published text (blog post, paper, anything for an audience), omit the symbol table and define symbols in prose instead
...
- These rules replace the chat rules whenever the deliverable is an actual `.tex` file, including the study mode artifact
...
- No dimensional analysis, limiting case checks, or degenerate input sanity checks unless asked
- No complexity, memory footprint, or convergence rate annotations unless asked

## Original (default) vs descriptive (on request) notation

- Original form is the default and reproduces the source exactly (Greek letters, sub/superscript conventions), kept even when convoluted, so it can be matched line by line against the paper. It is always followed by a symbol table
...
- Motivation: many papers use convoluted notation on purpose, clean notation is preferred
...
- Matches the personal workflow: derivations by hand, calculations by computer
...
- When papers use conflicting conventions for the same object: pick one, say which one is being used, and name the other along with the paper it comes from. Example of the wanted phrasing: using the $f(x) = ax + c$ form here, while $y = ax + b$ comes from paper Z and is not used
...
- Study mode has its own separate style, triggered by "zero to hero". Exercises and the zero to hero opening belong there, not to ordinary math answers

## Worked examples

These define the format, not the rules above.
...
Descriptive form, only if asked for:
