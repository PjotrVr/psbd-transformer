...
- Clean-label attacks (SIG, LC) use the custom `AttackSuccessSet`, not `FullyPoisonedTestSet` with `is_poisonable`.
...
Style and structure:
- Flat package and flat imports, PyTorch Lightning, no custom seeding.py.
...
- Unvalidated code belongs in `scratch/`, not `psbd/`.
