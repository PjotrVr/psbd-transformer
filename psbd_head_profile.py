...
Example
    python psbd_head_profile.py --checkpoint-folder vit_cifar10_badnet_a2o_0_01
"""
...
from defences.checkpoint_eval import PSBD_SPLIT_SEED
from defences.dropout import plug_dropout, unplug_dropout
from defences.inference import forward_probs
from defences.perturbations import fixed_head_mask
from defences.psbd_cache import load_or_build_baseline, write_split_manifest
from psbd_dropout_sweep import (
    current_git_commit,
...
def tracked_probs_with_head_ablated(
    ...
    """Confidence in each sample's baseline class, with one head deleted.
...
def already_complete(psbd_dir: str, splits) -> bool:
    return all(os.path.exists(profile_path(psbd_dir, split)) for split in splits)


def run_one_checkpoint(args, folder: str, device) -> None:
    psbd_dir = os.path.join(args.results_dir, folder, "psbd")
    model, architecture, loaders, manifest, _ = load_model_and_loaders(
        args, folder, device
    )
    if architecture != "vit":
