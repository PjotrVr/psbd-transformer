...
# The per-folder metrics are generated by the sweep and are not in git, so a fresh
# clone or a git worktree has none of them. Skipping keeps that an honest "no data
# here" rather than a failure that looks like a regression.
requires_generated_results = pytest.mark.skipif(
    not list(RESULTS_DIR.glob("*/psbd_metrics.json")),
    reason="no generated psbd_metrics.json under results/, run the sweep first",
)


@requires_generated_results
class TestResultsCoverage:
    """Verify the generated results cover the expected checkpoint set."""
