...
from defenses.decision import ADAPTIVE_SHIFT_TARGET, HEADLINE_QUANTILE

NOTEBOOK_DIRECTORY = Path("notebooks")

# Names a notebook must import rather than assign a literal to, mapped to the
# constant in defenses.decision that owns the value.
PROTOCOL_CONSTANTS = {
...
def test_protocol_constants_are_imported_not_restated(path):
    offences = [
        f"{name} = {value} (import {PROTOCOL_CONSTANTS[name][0]} from defenses.decision, "
...
def test_the_two_shift_targets_are_actually_different():
    ...
    from defenses.decision import SHIFT_MATCH_TARGETS
